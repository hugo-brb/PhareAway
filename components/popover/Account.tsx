...
export default function Account({ active, handleClickActive }: MenuProps) {
  ...
  const prenom = () => {
    let prenom = session?.user?.name?.split(" ");
    return prenom ? prenom[0] : "François";
  };

  const nom = () => {
    let nom = session?.user?.name?.split(" ");
    return nom ? nom[1].toUpperCase() : "SOLEIL";
  };

  const pseudo = () => {
    let pseudo = prenom().charAt(0) + nom();
    return pseudo ? pseudo.toLowerCase.toString : "Pharaon";
  };
        ...
        <section className=" flex flex-col self-center gap-12 w-fit h-fit max-h-[95vh] bg-white bg-opacity-60 rounded-3xl backdrop-blur-md mx-auto px-7 py-12 overflow-y-scroll scrollbarhidden">
            ...
            <div className=" flex flex-col gap-2">
              ...
              <div className=" flex justify-around">
                <div
                  ...
                  className=" flex items-center gap-2 border border-[--text] rounded-full mt-7 py-1 px-4 cursor-pointer hover:scale-110 duration-300"
                >
                  <svg
          ...
          <div className=" flex flex-col gap-12 pl-7 mx-auto px-7 py-12">
            ...
            <div className=" flex flex-col gap-1">
              ...
              <input
                type="mail"
                defaultValue={session?.user?.email ?? "phareaway@lighthouse.fr"}
                className=" py-2 px-4 w-full rounded-full bg-white bg-opacity-45"
                disabled
              />
            </div>
            <div className=" flex flex-col gap-1">
              ...
              <input
                type="password"
                defaultValue="Jaimelesphares38"
                className=" py-2 px-4 w-full rounded-full bg-white bg-opacity-45"
                disabled
              />
            </div>
            <button className=" w-[15vw] hover:bg-[--primary] hover:text-[--background] border-2 border-[--primary] duration-300 cursor-pointer text-xl font-bold mx-auto py-2 px-6 rounded-2xl">
              Modifier
            </button>
          </div>
        </section>
